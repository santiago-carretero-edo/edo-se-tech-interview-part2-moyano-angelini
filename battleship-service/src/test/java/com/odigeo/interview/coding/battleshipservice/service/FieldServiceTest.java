...
public class FieldServiceTest {
    ...
    public void testAllShipsSunk() {
        List<Coordinate> shipsCoordinates = shipsDeployment.stream()
                .map(Ship::getCoordinates)
                .flatMap(Collection::stream)
                .collect(toList());
        shipsCoordinates.forEach(shipCoordinate -> field[shipCoordinate.getRow()][shipCoordinate.getColumn()].hit());
        boolean areAllShipsSunk = fieldService.allShipsSunk(field);
        assertTrue(areAllShipsSunk);
    }

    @Test
    public void testNotAllShipsSunk() {
        List<Coordinate> shipsCoordinates = shipsDeployment.stream()
                .map(Ship::getCoordinates)
                .flatMap(Collection::stream)
                .collect(toList());
        shipsCoordinates.remove(shipsCoordinates.size()-1);
        shipsCoordinates.forEach(shipCoordinate -> field[shipCoordinate.getRow()][shipCoordinate.getColumn()].hit());
        boolean areAllShipsSunk = fieldService.allShipsSunk(field);
        assertFalse(areAllShipsSunk);
    }
